...
void GravityTDS::readKValues()
{
        ...
        EEPROM.read(this->kValueAddress + 3) == 0xFF)
    {
        ...
        EEPROM.commit();      // PATCH: wajib untuk ESP32
    }
}

void GravityTDS::calibration(const char* cmd)
{
    memset(cmdReceivedBuffer, 0, sizeof(cmdReceivedBuffer));
    strncpy(cmdReceivedBuffer, cmd, ReceivedBufferLength);
    cmdReceivedBuffer[ReceivedBufferLength] = '\0';
